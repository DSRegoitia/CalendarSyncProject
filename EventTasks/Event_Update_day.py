...
def sql_Queries(start,end):
    """
    connects to the DB using pyodbc and sends a SQL quiery and returns the inforamtion as a dataframe
    :return:
    """
    ...
    query = 'SELECT  [AppointmentID],[StartDate],[EndDate],[AppointmentName],[AssignedTo],[Description],[CreatedDate],' \
            '[CreatedBy]\
             FROM [DATABASE].[dbo].[Appointments]\
             WHERE StartDate BETWEEN \'' + start_date + '\' AND \''+ end_date + '\' ORDER BY StartDate ASC'
...
def sql_Quiery_with_user(start,end,user):
    cnxn = database_update()
    start_date = start
    end_date = end
    associate = user

    query = 'SELECT  [AppointmentID],[StartDate],[EndDate],[AppointmentName],[AssignedTo],[Description],' \
            '[CreatedDate],[CreatedBy] FROM [DATABASE].[dbo].[Appointments]\
             WHERE StartDate BETWEEN \'' + start_date + '\' AND \'' + end_date + '\' AND AssignedTo = \'' + user +\
...
def associates(asso):
    data = {'ID': ['35', '29', '34', '31', '32', '50', '30', '38', '39', '33', '0']}
    df = pd.DataFrame(data, index=['Carlos','Jackie', 'Laura','Monica','Mayra','Seyry', 'Silvia', 'OPEN','Diego','Elvin',
                                   'All'])
    user = df.at[asso, 'ID']
    print(user)
    print(type(user))
...
def appntmnts_info(*param):
    """
    using the returned dataframe form SQl_query function it gather the information of the appointment that will me used
    to create the google event.
    :return:
    """
    day = param[0]
    end = param[1]
    user = associates(param[2])
    print('in event day update the user received was'+user)

    if user != "0":
        df_appointments = sql_Quiery_with_user(day, end, user)

    else:
        df_appointments = sql_Queries(day, end)
...
def chkr(x, y):
    """
    the function check all events using the y variable which is the date the event was created. then it looks for all
    events after the y variable, looks for an event with a similar name (that's the x variable is)
    :return:
    """
    now = y
...
def reminders(x):
    """
    returns and int to be used as the reminder in the appointment info
