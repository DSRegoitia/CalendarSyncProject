...
def database_update():
    """
    Connects to the SQL database and retrieves the Appointment information.
    :return:
    """
...
def sql_Queries(date):
    ...
    query = 'SELECT  [AppointmentID],[StartDate],[EndDate],[AppointmentName],[AssignedTo],[Description],[CreatedDate],' \
            '[CreatedBy]\
             ...
             WHERE CreatedDate >= \'' + custom_date + '\' ORDER BY CreatedDate ASC'
...
def appntmnts_info(x):
    ...
    """
    date = x
    df_appointments = sql_Queries(date)
